    ...
    <div className="space-y-16">
      <div>
        ...
        <p className="mt-4 font-body text-lg text-muted-foreground leading-relaxed">
          Showcase dates, locations, and booking calls-to-action. Perfect for
          directory indexes or ticketing dashboards.
        </p>
      </div>
      ...
      <div className="space-y-6">
        <div>
          <h2 className="font-display text-2xl font-black uppercase tracking-tight">
            Variant A: Standard Grid Listing
          </h2>
          <p className="font-body text-sm text-muted-foreground mt-2">
            Provides thumbnail imagery alongside key logistical metadata
            context.
          </p>
        </div>
          ...
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <Card key={i} className="flex flex-col h-full bg-card group">
              <div className="relative aspect-[16/6] border-b-2 border-black overflow-hidden rounded-t-xl shrink-0">
                <Badge
                  variant="secondary"
                  className="absolute top-3 right-3 z-10 text-[10px] py-1 px-3 border-2 border-black shadow-brutal-sm uppercase"
                >
                  Conference
                </Badge>
                <img
              ...
              <CardHeader className="p-5 pb-2">
                <CardTitle className="text-xl line-clamp-2 leading-tight">
                  Neo-Brutalism Summit 2026 - Day {i}
                </CardTitle>
                <CardDescription className="text-muted-foreground font-semibold mt-2 line-clamp-2 leading-relaxed">
                  Gathering of the brightest minds in structural UI
                  architecture.
                </CardDescription>
              </CardHeader>
      ...
      <div className="space-y-6">
        <div>
          <h2 className="font-display text-2xl font-black uppercase tracking-tight">
            Variant B: Minimal List Index (Horizontal)
          </h2>
          <p className="font-body text-sm text-muted-foreground mt-2">
            High density for calendar views. Recommended for single or
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 w-full">
          {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
            <Card
              key={i}
              className="flex flex-col sm:flex-row items-center border-l-[8px] border-l-secondary bg-card p-4 sm:p-5 gap-4 sm:gap-6 hover:bg-muted/50 transition-colors"
            >
              ...
              <Button
                variant="outline"
                size="sm"
                className="w-full sm:w-auto shrink-0 font-bold border-black"
              >
                RSVP
              </Button>
            </Card>
