...
import { Calendar, MapPin, Users, Ticket, ArrowLeft, ArrowRight, Share2, Tag, AlertTriangle, AlertCircle } from "lucide-react";
  ...
  if (!eventItem) {
    return (
      <div className="w-full min-h-[50vh] flex flex-col items-center justify-center p-8 text-center text-foreground">
        <h1 className="text-4xl font-display font-black uppercase mb-4">404: Event Tidak Ditemukan</h1>
    ...
    <div className="w-full min-h-screen bg-background">
      ...
      <section className="w-full bg-card border-b-2 border-black relative overflow-hidden">
        <div className="w-full h-[40vh] md:h-[50vh] xl:h-[60vh] relative bg-muted z-0">
          <img src={eventItem.thumbnail} alt={eventItem.title} className={cn("w-full h-full object-cover", !isUpcoming && "grayscale")} />
      ...
      <section className="w-full py-12 md:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
           ...
           <div className="flex items-center gap-2 mb-8 text-muted-foreground hover:text-primary transition-colors">
              ...
              <Link to="/events" className="font-body font-bold text-sm uppercase">Kembali ke Daftar Acara</Link>
           </div>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start relative">
              ...
              <div className="lg:col-span-8 flex flex-col gap-12">
                 ...
                 <article className="prose prose-lg dark:prose-invert max-w-none font-body prose-headings:font-display prose-headings:font-black prose-headings:uppercase prose-headings:tracking-tighter prose-a:text-primary prose-a:font-bold">
                    ...
                    <div className="markdown-body" dangerouslySetInnerHTML={{ __html: eventItem.body_html }} />
                 </article>
                 
                 {/* Tags */}
                 {eventItem.tags && eventItem.tags.length > 0 && (
                   <div className="pt-8 border-t-2 border-dashed border-black">
                     <h4 className="font-body font-bold uppercase mb-4 text-sm text-foreground flex items-center gap-2"><Tag className="w-4 h-4"/> Tag Terkait</h4>
                     <div className="flex flex-wrap gap-2">
                       {eventItem.tags.map(t => (
                         <span key={t} className="px-3 py-1 bg-muted border-2 border-black rounded-full text-xs font-bold font-body text-foreground uppercase">{t}</span>
                       ))}
                     </div>
                   </div>
                 )}
                 ...
                 <div className="mt-8 p-8 border-4 border-black rounded-2xl bg-primary text-black shadow-brutal flex flex-col">
                   ...
                   {!isUpcoming ? (
                      <div className="bg-black text-white p-6 rounded-xl border-2 border-black text-center flex flex-col items-center">
                        ...
                        <span className="font-display font-black text-2xl uppercase">OPERASI SELESAI</span>
                        <p className="font-body font-medium mt-2 text-sm text-gray-400">Acara ini telah berhasil dilaksanakan dan masuk ke dalam arsip portofolio kami.</p>
                      </div>
                   ) : (
                      <TicketingPanel eventId={eventItem.id} ticketing={(eventItem as any).ticketing} price={String(eventItem.price)} />
                   )}
                 </div>
                 
                 <div className="mt-8 flex justify-center">
                   <Button variant="outline" className="rounded-xl border-2 border-black border-dashed flex items-center gap-2 hover:bg-black hover:text-white transition-colors bg-transparent text-foreground">
...
function EventMetas({ item, isUpcoming }: { item: any, isUpcoming: boolean }) {
  return (
    <>
      <h3 className="font-display font-black text-xl uppercase border-b-2 border-black pb-2 text-foreground">Detail Operasi</h3>
      <div className="flex flex-col gap-4 font-body font-bold text-sm text-foreground">
         <div className="flex items-start gap-4">
            <Calendar className="w-6 h-6 text-primary shrink-0" />
            <div className="flex flex-col">
              <span className="text-muted-foreground text-xs">JADWAL PELAKSANAAN</span>
         ...
         <div className="flex items-start gap-4">
            ...
            <div className="flex flex-col">
              <span className="text-muted-foreground text-xs">KOORDINAT LOKASI</span>
         ...
         {isUpcoming ? (
           ...
           {item.capacity && (
            <div className="flex items-start gap-4">
                ...
                <div className="flex flex-col">
                  <span className="text-muted-foreground text-xs">KAPASITAS MAKSIMUM</span>
           ...
           <div className="flex items-start gap-4">
              ...
              <div className="flex flex-col">
                <span className="text-muted-foreground text-xs">INFO TIKET</span>
           ...
           <div className="flex items-start gap-4">
              ...
              <div className="flex flex-col">
                <span className="text-muted-foreground text-xs">TOTAL KEHADIRAN</span>
                <span className="text-xl font-black">{item.attendance?.toLocaleString()} Pax</span>
              </div>
           </div>
         )}
      </div>
...
function TicketingPanel({ eventId, ticketing, price }: { eventId: string, ticketing: any, price: string }) {
  if (!ticketing || !ticketing.available) {
    return (
       <div className="bg-red-500 text-white p-6 rounded-xl border-2 border-black text-center">
          <span className="font-display font-black text-2xl uppercase tracking-tighter">SOLD OUT</span>
          <p className="font-body font-medium mt-2 text-sm opacity-90">Kapasitas telah mencapai batas aman. Tidak ada tiket tambahan.</p>
       </div>
    );
  }
    ...
    <div className="flex flex-col gap-6">
      {ticketing.ticket_types.map((opt: any) => {
        ...
        <div key={opt.id} className={cn(
          ...
          isSoldOut ? "opacity-60 bg-gray-200 cursor-not-allowed" : "hover:-translate-y-1 hover:shadow-brutal-sm cursor-pointer"
        )}>
          <div className="flex justify-between items-start mb-2">
            <h4 className="font-display font-black text-xl uppercase">{opt.name}</h4>
            <span className="font-body font-bold text-lg">Rp{(opt.price).toLocaleString('id-ID')}</span>
          </div>
          
          <div className="flex items-center gap-2 font-body text-xs font-bold uppercase mt-4">
            {isSoldOut ? (
              <span className="bg-red-600 text-white px-2 py-1 rounded">HABIS TERJUAL</span>
            ) : lowStock ? (
              <span className="bg-orange-500 text-white px-2 py-1 rounded flex items-center gap-1"><AlertCircle className="w-3 h-3"/> SISA {sisa} TIKET LAGI!</span>
            ) : (
              <span className="bg-green-500 text-white px-2 py-1 rounded">TERSEDIA</span>
            )}
            
            {opt.type === "member_only" && (
              <span className="bg-blue-600 text-white px-2 py-1 rounded">MEMBER ONLY</span>
            )}
          </div>
          ...
          {!isSoldOut && (
            <div className="mt-4 border-t-2 border-black pt-4">
               ...
               <CheckoutFlowModal eventId={eventId} ticketId={opt.id} ticketName={opt.name} />
            </div>
          )}
        </div>
      )})}
...
function CheckoutFlowModal({ eventId, ticketId, ticketName }: { eventId: string, ticketId: string, ticketName: string }) {
      ...
      <DialogContent className="sm:max-w-md border-4 border-black rounded-2xl shadow-brutal p-8 bg-card">
        ...
        <div className="flex flex-col gap-6 mt-4">
          <p className="font-body font-medium text-muted-foreground text-sm">
            Anda akan memesan tiket <strong>{ticketName}</strong>. Masukkan alamat email untuk melanjutkan ke area pemrosesan pembayaran.
          </p>
          <div className="space-y-2">
            <Label htmlFor="email" className="font-display font-extrabold uppercase">Alamat Surel / Email</Label>
            <Input 
              id="email" 
              type="email" 
          ...
          }} className={cn("w-full h-14 rounded-xl font-bold uppercase border-2", email ? "bg-primary text-black border-black shadow-brutal-sm hover:translate-y-1 hover:shadow-none" : "bg-muted text-muted-foreground border-transparent pointer-events-none")}>
            <Link to={`/checkout/${eventId}?ticket=${ticketId}&email=${encodeURIComponent(email)}`}>
