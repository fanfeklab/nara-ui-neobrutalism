      ...
      <section className="w-full py-20 bg-background  border-b-2 border-black">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="text-5xl md:text-7xl font-display font-black uppercase tracking-tighter mb-6 relative inline-block text-foreground">
            LAYANAN{" "}
            <span className="text-primary drop-shadow-[2px_2px_0_#000] dark:drop-shadow-none bg-black px-4 ml-2 border-4 border-black dark:border-white rounded-xl">
              KAMI
            </span>
          </h1>
          <p className="text-xl font-body max-w-2xl text-muted-foreground font-medium">
            Dari sketsa kasar hingga gemuruh tepuk tangan. Kami orkestrasikan
            semuanya.
          </p>
        </div>
      </section>
      ...
      <div className="flex flex-col">
        {serviceCategories.map((category, index) => (
          <section
            ...
            className={cn(
              "w-full py-16 md:py-24 border-b-2 border-black",
              index % 2 === 0
                ? "bg-card"
                : "bg-card text-card-foreground text-white dark:bg-card dark:text-foreground",
            )}
          >
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                {category.items.map((item) => {
                  ...
                  return (
                    <div
                      key={item.id}
                      className="bg-card text-card-foreground border-4 border-black rounded-2xl shadow-brutal hover:-translate-y-2 hover:shadow-[8px_8px_0_0_#000] transition-all flex flex-col group"
                    >
                      <div className="p-6 md:p-8 flex flex-col flex-1">
                        ...
                        <div className="mt-auto border-t-2 border-black pt-4">
                          <Link
                            to={item.slug}
                            className="flex items-center gap-2 font-display font-black uppercase text-lg text-black hover:text-accent transition-colors"
                          >
                            LIHAT DETAIL{" "}
