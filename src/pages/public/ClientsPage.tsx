      ...
      <section className="w-full py-20 bg-card text-card-foreground text-white border-b-2 border-black relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          <p className="text-xl font-body font-medium text-gray-300">
            Daftar merek dan institusi yang telah membuktikan standar presisi
            operasional kami.
          </p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {our_clients.map((client, index) => (
              <div
                key={index}
                className="aspect-[3/2] bg-card border-4 border-black rounded-2xl shadow-brutal flex items-center justify-center p-8 group hover:-translate-y-2 hover:bg-primary transition-colors"
              >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-display font-black uppercase tracking-tighter mb-4 flex items-center justify-center gap-4">
              <MessageSquare className="w-10 h-10 text-accent p-1 border-2 border-black rounded shadow-brutal-sm bg-card" />{" "}
              DEKLARASI KEPUASAN
            </h2>
            <p className="font-body text-xl text-muted-foreground font-medium max-w-2xl mx-auto">
              Dengarkan langsung dari mereka yang telah merasakan presisi
              operasional kami.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {testimonials.map((item) => (
              <div
                key={item.id}
                className="bg-card  border-4 border-black rounded-2xl p-8 shadow-brutal flex flex-col relative group"
              >
                <div className="absolute -top-6 -left-6 text-6xl text-primary drop-shadow-[2px_2px_0_#000] z-0 font-display font-black opacity-80 group-hover:rotate-12 transition-transform">
                  "
                </div>
                ...
                <div className="mt-auto border-t-2 border-black pt-4 flex items-center gap-4">
                  ...
                  <div className="flex flex-col">
                    <span className="font-display font-black uppercase text-sm leading-tight text-foreground">
                      {item.name}
                    </span>
                    <span className="font-body text-xs font-bold text-muted-foreground uppercase">
                      {item.role}
                    </span>
                  </div>
                </div>
