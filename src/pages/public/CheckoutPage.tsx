...
export default function CheckoutPage() {
  ...
  const ticketOption = (eventItem as any)?.ticketing?.ticket_types?.find(
    (o: any) => o.id === ticketId,
  );
  const hasSeatAssignment =
    (eventItem as any)?.ticketing?.has_seat_assignment === true;
    ...
    return (
      <div className="w-full min-h-[50vh] flex flex-col items-center justify-center p-8 text-center bg-background text-foreground">
        ...
        <p className="font-body text-muted-foreground mb-8">
          Reservasi tidak ditemukan atau waktu telah habis.
        </p>
        <Button
          asChild
          variant="outline"
      ...
      <div className="w-full min-h-[80vh] flex flex-col items-center justify-center p-8 text-center bg-background  text-foreground">
        <div className="bg-card border-4 border-black rounded-3xl p-8 md:p-12 shadow-brutal flex flex-col items-center max-w-lg w-full">
          ...
          <p className="font-body text-muted-foreground font-medium mb-8">
            E-Ticket telah dikirimkan ke <strong>{formData.email}</strong>.
            Tunjukkan kode QR pada petugas saat memasuki area perimeter acara.
          </p>
          <div className="w-full p-6 bg-muted border-2 border-dashed border-black rounded-xl flex flex-col gap-2 mb-8 text-left font-body text-sm font-bold">
            <div className="flex justify-between">
              <span className="text-muted-foreground uppercase">Order ID:</span>{" "}
              <span>NARA-{Math.floor(Math.random() * 1000000)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground uppercase">Tiket:</span>{" "}
              <span>{ticketOption.name}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground uppercase">Acara:</span>{" "}
              <span>{eventItem.title}</span>
            </div>
            {hasSeatAssignment && (
              <div className="flex justify-between">
                <span className="text-muted-foreground uppercase">Kursi:</span>{" "}
                <span className="text-primary">{formData.seat}</span>
              </div>
            )}
          </div>
          <Button
            asChild
            variant="solid"
            className="w-full h-14 rounded-xl font-bold uppercase text-lg border-2 border-black bg-primary text-black hover:bg-black hover:text-white shadow-brutal-sm transition-all"
          >
        ...
        >
          ...
          <div className="lg:col-span-7 flex flex-col gap-10">
            ...
            {ticketOption.type === "member_only" && (
              <div className="p-4 bg-blue-100 border-2 border-blue-600 rounded-xl flex items-start gap-4">
                ...
                <div>
                  <h4 className="font-display font-black text-blue-900 uppercase">
                    TIKET MEMBER EKSKLUSIF
                  </h4>
                  <p className="font-body text-sm text-blue-800 font-medium">
                    Sistem akan melakukan verifikasi NIK Anda secara otomatis
                    dengan database Member {BRAND.name}. Pastikan NIK yang Anda
                    masukkan sesuai dengan identitas member Anda.
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="bg-card border-4 border-black rounded-2xl p-6 md:p-8 shadow-brutal">
              <h3 className="font-display font-black text-2xl uppercase tracking-tight mb-6 border-b-2 border-black pb-2">
                Informasi Pemesan
              </h3>
            ...
            {hasSeatAssignment && (
              <div className="bg-card border-4 border-black rounded-2xl p-6 md:p-8 shadow-brutal">
                <div className="flex items-center justify-between mb-6 border-b-2 border-black pb-2">
                  ...
                  {formData.seat && (
                    <span className="bg-primary text-black font-body font-bold px-3 py-1 border-2 border-black rounded-lg">
                      {formData.seat}
                    </span>
                  )}
                </div>

                <p className="font-body text-sm font-medium text-muted-foreground mb-6">
                  Pilih lokasi kursi yang tersedia.
                </p>
                ...
                <div className="w-full bg-muted border-2 border-black rounded-xl p-4 overflow-x-auto hide-scrollbars">
                  <div className="min-w-[400px] flex flex-col gap-4">
                    <div className="w-full h-8 bg-black text-white text-xs font-display font-black flex items-center justify-center rounded uppercase tracking-widest mb-4">
                      PANGGUNG UTAMA
                    </div>

                    {["A", "B", "C"].map((row) => (
                      <div key={row} className="flex justify-center gap-2">
                        <div className="w-8 flex items-center font-display font-black">
                          {row}
                        </div>
                            ...
                            <button
                              ...
                              className={cn(
                                ...
                                isSold
                                  ...
                                  : isSelected
                                    ? "bg-primary text-black shadow-brutal-sm -translate-y-1"
            ...
            <div className="bg-card border-4 border-black rounded-2xl p-6 md:p-8 shadow-brutal">
              ...
              >
                <div
                  className={cn(
                    "p-4 border-2 border-black rounded-xl flex items-start gap-3 cursor-pointer transition-colors relative",
                ...
                >
                  ...
                  <div className="flex flex-col">
                    <Label
                      htmlFor="qris"
                      className="font-display font-black uppercase text-base cursor-pointer"
                    >
                      QRIS
                    </Label>
                    <p className="font-body text-xs font-bold text-gray-600 mt-1">
                      Scan otomatis dengan e-wallet/m-banking (Bebas Biaya
                      Admin)
                    </p>
                  </div>
                </div>
                ...
                <div
                  ...
                  onClick={() =>
                    setFormData({ ...formData, paymentMethod: "va" })
                  }
                >
                  ...
                  <div className="flex flex-col">
                    ...
                    <p className="font-body text-xs font-bold text-gray-600 mt-1">
                      Transfer via bank BCA, Mandiri, BNI, BRI.
                    </p>
                  </div>
                </div>
                ...
                <div
                  ...
                  onClick={() =>
                    setFormData({ ...formData, paymentMethod: "cc" })
                  }
                >
                  ...
                  <div className="flex flex-col">
                    ...
                    <p className="font-body text-xs font-bold text-gray-600 mt-1">
                      Visa/Mastercard dengan biaya tambahan Rp 5.000
                    </p>
                  </div>
                </div>
              </RadioGroup>
          ...
          <div className="lg:col-span-5 lg:sticky lg:top-28">
            <div className="bg-card text-card-foreground text-white border-4 border-black rounded-2xl p-6 md:p-8 shadow-brutal flex flex-col gap-6">
              <h3 className="font-display font-black text-2xl uppercase tracking-tight border-b-2 border-gray-600 pb-4">
                RINGKASAN PESANAN
              </h3>
              ...
              <div className="flex gap-4 items-center">
                ...
                <div className="flex flex-col">
                  <span className="font-body font-bold text-xs text-primary uppercase mb-1">
                    {eventItem.category}
                  </span>
                  <h4 className="font-display font-black uppercase leading-tight line-clamp-2">
                    {eventItem.title}
                  </h4>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3 font-body text-sm font-medium border-y-2 border-gray-600 py-6">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-bold">
                    Rp{ticketOption.price.toLocaleString("id-ID")}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400 uppercase">Pajak (11%)</span>
                  <span className="font-bold">
                    Rp{(ticketOption.price * 0.11).toLocaleString("id-ID")}
                  </span>
                </div>
                <div className="flex justify-between">
              ...
              <div className="flex justify-between items-end">
                <span className="font-display font-black text-lg uppercase text-gray-400">
                  TOTAL BAYAR
                </span>
                <span className="font-display font-black text-3xl text-primary">
                  Rp
                  {(
                    ticketOption.price * 1.11 +
                    5000 +
                    (formData.paymentMethod === "cc" ? 5000 : 0)
              ...
              {hasSeatAssignment && !formData.seat && (
                <p className="font-body text-xs text-red-400 font-bold text-center mt-2">
                  *Wajib memilih kursi terlebih dahulu.
                </p>
              )}
            </div>
          </div>
