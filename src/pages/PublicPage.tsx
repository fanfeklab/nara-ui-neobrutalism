...
export default function PublicPage() {
  return (
    <div className="w-full">
        ...
        <div className="container relative z-10 mx-auto px-4 py-24 md:py-32 lg:py-40 flex flex-col items-center text-center">
          ...
          <div className="mt-12 flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
            <Button asChild size="lg" variant="solid" className="w-full sm:w-auto text-lg h-16 px-8 rounded-2xl">
              <Link to="/events">EKSPLORASI ACARA <ArrowRight className="ml-2 w-5 h-5" /></Link>
            </Button>
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
            ...
            <Button asChild variant="outline" className="rounded-xl font-bold uppercase">
              <Link to="/events">Lihat Semua Acara <ArrowRight className="w-4 h-4 ml-2" /></Link>
            </Button>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {/* 1 Big Featured Card */}
            <div className="lg:col-span-2 group bg-background border-4 border-black rounded-2xl overflow-hidden shadow-brutal flex flex-col hover:-translate-y-2 hover:shadow-[12px_12px_0_0_#000] transition-all">
              <div className="relative aspect-video lg:aspect-auto border-b-4 border-black overflow-hidden flex-1 min-h-[300px]">
                <img src="https://picsum.photos/seed/naramain/1200/600" alt="Main Event" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                <div className="absolute top-4 left-4 bg-[#8a2be2] text-white font-display font-black text-lg uppercase px-4 py-2 border-2 border-black rounded-xl shadow-brutal-sm">
                  FESTIVAL
                </div>
                {/* Sold Out / Status badge mockup */}
                <div className="absolute top-4 right-4 bg-[#ccff00] text-black font-body font-bold text-xs uppercase px-3 py-1 border-2 border-black rounded-lg">
                  Tiket Tersedia
                </div>
              </div>
              <div className="p-8 bg-white dark:bg-[#112240] flex flex-col gap-4">
                <h3 className="text-3xl font-display font-black uppercase tracking-tighter">Neo-Brutalism Tech Summit 2026</h3>
                <div className="flex flex-wrap items-center gap-6 font-body text-sm font-bold text-muted-foreground">
                  <span className="flex items-center gap-2"><Calendar className="w-4 h-4 text-black dark:text-white" /> 24-26 AGUSTUS 2026</span>
                  <span className="flex items-center gap-2"><MapPin className="w-4 h-4 text-black dark:text-white" /> JCC SENAYAN, JAKARTA</span>
                </div>
                <p className="font-body font-medium mt-2 mb-4 line-clamp-2">
            ...
            <div className="flex flex-col gap-8">
              {[1, 2].map((item) => (
                <div key={item} className="flex flex-col bg-background border-4 border-black rounded-2xl overflow-hidden shadow-brutal hover:-translate-y-1 transition-transform group flex-1">
                  <div className="h-40 border-b-4 border-black overflow-hidden relative">
                  ...
                  <div className="p-5 flex flex-col justify-between flex-1 bg-white dark:bg-[#112240]">
                    <div>
                      <h4 className="text-xl font-display font-black uppercase tracking-tight mb-2 leading-tight">
                        {item === 1 ? "Underground Synth Rave" : "Startup Pitch Battle Vol.4"}
                      </h4>
                      <div className="flex flex-col gap-1 font-body text-xs font-bold text-muted-foreground mb-4">
                        <span className="flex items-center gap-1"><Calendar className="w-3 h-3" /> 1{item} September 2026</span>
                      </div>
                    </div>
                    <Button variant="outline" className="w-full rounded-xl font-bold uppercase text-xs">
                      Lihat Info
                    </Button>
                  </div>
                </div>
      ...
      <section className="w-full py-24 bg-card border-b-2 border-black overflow-hidden">
        <div className="container mx-auto px-4">
           ...
           <div className="flex overflow-x-auto gap-6 pb-8 snap-x snap-mandatory hide-scrollbars">
             {[1, 2, 3, 4].map((i) => (
                <div key={i} className="shrink-0 w-[85vw] md:w-[400px] bg-background border-4 border-black rounded-2xl snap-center overflow-hidden shadow-brutal pb-6">
