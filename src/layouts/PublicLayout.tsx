        ...
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
                    ...
                    <div className="bg-card border-2 border-black shadow-brutal flex flex-col p-2 rounded-xl">
                      {item.children.map((child) => (
                        <Link
                          key={child.label}
                          to={child.url}
                          className="px-4 py-2 hover:bg-primary hover:text-primary-foreground rounded-lg transition-colors font-bold whitespace-nowrap"
                        >
                          {child.label}
                        </Link>
                      ))}
          ...
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <Button
              asChild
              variant="outline"
              className="hidden lg:flex rounded-xl font-bold h-10 md:h-12 border-2 border-black dark:bg-card"
            >
            ...
            <Button
              asChild
              variant="solid"
              className="hidden md:flex rounded-xl font-bold h-10 md:h-12"
            >
              <Link to={cta_button.url}>
                {cta_button.label} <ArrowRight className="w-4 h-4 ml-2" />
              </Link>
            </Button>

            {/* Mobile Menu Toggle */}
            <button
              className="lg:hidden p-2 bg-primary text-primary-foreground border-2 border-black rounded-xl shadow-brutal-sm h-10 w-10 md:h-12 md:w-12 flex items-center justify-center"
      ...
      >
        ...
        >
          <div className="flex items-center justify-between mb-8">
            ...
            <button
              className="p-2 bg-muted text-foreground border-2 border-black rounded-xl shadow-brutal-sm hover:bg-destructive hover:text-destructive-foreground transition-colors"
              onClick={() => setMobileMenuOpen(false)}
            >
              <X className="w-6 h-6" />
            </button>
          ...
          <div className="flex flex-col gap-6 font-display font-bold uppercase tracking-tight text-lg mb-8">
            <Link
              to="/"
              className="pb-2 border-b-2 border-muted hover:text-primary"
            >
              Beranda
            </Link>
          ...
          <div className="mt-auto pt-8 border-t-2 border-black flex flex-col gap-4">
            ...
            <Button
              asChild
              variant="solid"
              className="w-full justify-center rounded-xl h-12 text-lg"
            >
              <Link to={cta_button.url}>
                {cta_button.label.toUpperCase()}{" "}
      ...
      <footer className="border-t-2 border-black bg-card  mt-auto w-full">
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
            ...
            {/* Contact */}
            <div className="space-y-6">
              <h4 className="font-display font-black text-xl uppercase tracking-tighter border-b-2 border-black pb-2 inline-block">
                Hubungi Kami
              </h4>
            ...
            <div className="space-y-6 md:col-span-2 lg:col-span-1">
              ...
              <p className="font-body text-sm font-bold text-muted-foreground mb-4">
                ...
                {Object.entries(footer.social_media).map(([platform, url]) => {
                  ...
                  return (
                    <a
                      ...
                      className="w-12 h-12 bg-card text-foreground flex items-center justify-center rounded-xl border-2 border-black hover:-translate-y-1 hover:bg-primary hover:text-primary-foreground transition-all shadow-brutal-sm group"
                    >
