...
export function CodeBlock({
  ...
  return (
    <div
      className={cn(
        "relative group rounded-2xl border-2 border-black shadow-brutal overflow-hidden flex flex-col mb-4 bg-black",
        className,
      )}
      {...props}
    >
      ...
      <div className="flex items-center justify-between bg-black text-white px-4 py-3 font-mono text-xs border-b-2 border-border shrink-0">
        ...
        <div className="flex items-center gap-3">
          <span className="opacity-80 font-bold lowercase tracking-wider">
            {language}
          </span>
          <button
        ...
        >
          {({ className, style, tokens, getLineProps, getTokenProps }) => (
            <pre
              className={cn(
                className,
                "p-4 overflow-x-auto font-mono text-sm leading-relaxed text-left m-0",
              )}
